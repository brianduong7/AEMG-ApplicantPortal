/** Recruiter portal chrome: AIFE palette, white logo strip, navy sidebar. */
...
export const RECRUITER_PORTAL_SHELL_THEME: RecruiterShellTheme = {
  logoStrip:
    "flex shrink-0 items-center justify-center gap-2 border-b border-slate-200/90 bg-white px-3 py-4 transition hover:bg-slate-50/90",
...
/**
 * Per-company sidebar chrome (AEMG vs AIFE) reused by the applicant portal layout.
 * The live recruiter desk always uses {@link RECRUITER_PORTAL_SHELL_THEME}.
 */
export const RECRUITER_SHELL_THEME: Record<CompanyId, RecruiterShellTheme> = {
  ...
  aife: RECRUITER_PORTAL_SHELL_THEME,
};

export function getRecruiterShellTheme(): RecruiterShellTheme {
  return RECRUITER_PORTAL_SHELL_THEME;
}
