...
import {
  addJobApplicantComment,
  type ApplicantCommentFormState,
} from "@/app/actions/applicant-comment";

type Props = {
  applicantDocName: string;
};

export function ApplicantCommentForm({ applicantDocName }: Props) {
  const [state, formAction, pending] = useActionState(
    addJobApplicantComment,
    null as ApplicantCommentFormState,
  );
  ...
  return (
    <form ref={formRef} action={formAction} className="flex flex-col gap-3 border-t border-slate-100 pt-4">
      <input type="hidden" name="applicantDocName" value={applicantDocName} />
      <label htmlFor="applicant-comment" className="text-sm font-medium text-slate-700">
        Add comment
      </label>
      <textarea
        ref={textareaRef}
        ...
        required
        rows={5}
        placeholder="Write a note for this applicant…"
