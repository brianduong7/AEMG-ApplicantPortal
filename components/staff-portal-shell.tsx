"use client";

import Link from "next/link";
import type { ReactNode } from "react";
import { useMemo, useState } from "react";
import { usePathname } from "next/navigation";
import { logout } from "@/app/actions/auth";
import { RecruiterBrandLogos } from "@/components/recruiter-brand-logos";
import { hrGreetingNameFromEmail } from "@/lib/hr-display-name";
import type { RecruiterShellTheme } from "@/lib/recruiter-shell-theme";
import { getRecruiterShellTheme } from "@/lib/recruiter-shell-theme";
import { STAFF_PORTAL_BASE } from "@/lib/staff-portal-base";
import { staffRolesAllow, type StaffPortalRole } from "@/lib/staff-portal-role";

/** Client-safe copy of staffUseScopedDepartmentManagerDataPlane (avoids server-only imports). */
function isDepartmentManagerOnlyNav(roles: StaffPortalRole[]): boolean {
  if (roles.includes("super_admin")) return false;
  const wide = staffRolesAllow(roles, ["d_recruiter", "d_hr", "d_executive"]);
  return staffRolesAllow(roles, ["d_department_manager"]) && !wide;
}

type NavLinkItem = {
  href: string;
  label: string;
  match: (path: string) => boolean;
  icon: ReactNode;
  roles: readonly StaffPortalRole[];
};

function IconDashboard({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M4 10.5 12 4l8 6.5V20a1 1 0 0 1-1 1h-5v-6H10v6H5a1 1 0 0 1-1-1v-9.5Z" strokeLinejoin="round" />
    </svg>
  );
}

function IconBriefcase({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <rect x="3" y="7" width="18" height="13" rx="2" />
      <path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
    </svg>
  );
}

function IconUsers({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
      <circle cx="9" cy="7" r="4" />
      <path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" />
    </svg>
  );
}

function IconCalendar({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <rect x="3" y="4" width="18" height="18" rx="2" />
      <path d="M16 2v4M8 2v4M3 10h18" />
    </svg>
  );
}

function IconClipboardList({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <rect x="8" y="2" width="8" height="4" rx="1" ry="1" />
      <path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2" />
      <path d="M12 11h4M12 16h4M8 11h.01M8 16h.01" strokeLinecap="round" />
    </svg>
  );
}

function IconBadgeCheck({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z" />
      <path d="m9 12 2 2 4-4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function IconLogout({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9" />
    </svg>
  );
}

function IconBell({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" />
      <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0" />
    </svg>
  );
}

function IconChevron({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      <path d="m6 9 6 6 6-6" />
    </svg>
  );
}

function IconMenu({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      <path d="M4 6h16M4 12h16M4 18h16" />
    </svg>
  );
}

const base = STAFF_PORTAL_BASE;

const NAV_LINKS: NavLinkItem[] = [
  {
    href: `${base}/dashboard`,
    label: "Dashboard",
    match: (p) => p === base || p === `${base}/dashboard`,
    icon: <IconDashboard className="h-5 w-5 shrink-0" />,
    roles: ["d_recruiter", "d_hr", "d_executive", "super_admin"],
  },
  {
    href: `${base}/job-requisitions`,
    label: "Job requisitions",
    match: (p) => p.startsWith(`${base}/job-requisitions`),
    icon: <IconClipboardList className="h-5 w-5 shrink-0" />,
    roles: [
      "d_department_manager",
      "d_recruiter",
      "d_hr",
      "d_executive",
      "super_admin",
    ],
  },
  {
    href: `${base}/openings`,
    label: "Job openings",
    match: (p) => p.startsWith(`${base}/openings`),
    icon: <IconBriefcase className="h-5 w-5 shrink-0" />,
    roles: ["d_recruiter", "d_hr", "d_executive", "super_admin"],
  },
  {
    href: `${base}/applicants`,
    label: "Job applicants",
    match: (p) => p.startsWith(`${base}/applicants`),
    icon: <IconUsers className="h-5 w-5 shrink-0" />,
    roles: ["d_recruiter", "d_hr", "d_executive", "super_admin"],
  },
  {
    href: `${base}/interviews`,
    label: "Interviews",
    match: (p) => p.startsWith(`${base}/interviews`),
    icon: <IconCalendar className="h-5 w-5 shrink-0" />,
    roles: ["d_recruiter", "super_admin"],
  },
  {
    href: `${base}/job-offers`,
    label: "Job offers",
    match: (p) => p.startsWith(`${base}/job-offers`),
    icon: <IconBadgeCheck className="h-5 w-5 shrink-0" />,
    roles: ["d_recruiter", "d_hr", "d_executive", "super_admin"],
  },
];

function SidebarNavLinks({
  pathname,
  onNavigate,
  theme,
  roles,
}: {
  pathname: string;
  onNavigate?: () => void;
  theme: RecruiterShellTheme;
  roles: StaffPortalRole[];
}) {
  const dmOnly = isDepartmentManagerOnlyNav(roles);
  const links = NAV_LINKS.filter((item) => staffRolesAllow(roles, item.roles)).filter(
    (item) => !dmOnly || item.href.includes("/job-requisitions"),
  );

  return (
    <>
      {links.map((item) => {
        const active = item.match(pathname);
        return (
          <Link
            key={item.href}
            href={item.href}
            onClick={onNavigate}
            className={`flex items-center gap-3 rounded-lg py-2.5 pl-3 pr-3 text-sm transition ${
              active ? theme.navLinkActive : theme.navLinkInactive
            }`}
          >
            {item.icon}
            {item.label}
          </Link>
        );
      })}
    </>
  );
}

type Props = {
  children: React.ReactNode;
  email: string;
  roles: StaffPortalRole[];
};

export function StaffPortalShell({ children, email, roles }: Props) {
  const pathname = usePathname() ?? "";
  const t = getRecruiterShellTheme();
  const greeting = hrGreetingNameFromEmail(email);
  const [navOpen, setNavOpen] = useState(false);
  const roleKey = useMemo(() => roles.join(","), [roles]);
  const homeHref =
    isDepartmentManagerOnlyNav(roles) ?
      `${base}/job-requisitions`
    : `${base}/dashboard`;

  return (
    <div className="min-h-dvh" key={roleKey}>
      {navOpen ?
        <button
          type="button"
          className="fixed inset-0 z-30 bg-slate-900/40 md:hidden"
          aria-label="Close menu"
          onClick={() => setNavOpen(false)}
        />
      : null}

      <aside
        className={`${t.asideOuter} bg-white ${
          navOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <Link href={homeHref} onClick={() => setNavOpen(false)} className={t.logoStrip}>
          <RecruiterBrandLogos layout="sidebar" />
        </Link>

        <div className={t.sidebarBody}>
          <nav className="flex flex-1 flex-col space-y-0.5 overflow-y-auto px-3 py-4">
            <SidebarNavLinks
              pathname={pathname}
              onNavigate={() => setNavOpen(false)}
              theme={t}
              roles={roles}
            />
          </nav>

          <div className={t.footerBar}>
            <form action={logout}>
              <button type="submit" className={t.logoutButton}>
                <IconLogout className="h-5 w-5 shrink-0 opacity-80" />
                Log out
              </button>
            </form>
          </div>
        </div>
      </aside>

      <div className={t.mainColumn}>
        <header className={t.headerBar}>
          <button
            type="button"
            className={t.mobileMenuBtn}
            aria-label="Open menu"
            onClick={() => setNavOpen(true)}
          >
            <IconMenu className="h-6 w-6" />
          </button>

          <div className="ml-auto flex shrink-0 items-center gap-3">
            <button type="button" className={t.notifyBtn} aria-label="Notifications">
              <IconBell className="h-5 w-5" />
              <span className={t.notifyBadge}>3</span>
            </button>
            <div className={t.profileChip}>
              <span className={t.profileAvatar}>{greeting.slice(0, 1)}</span>
              <span className="max-w-[12rem] truncate text-sm font-medium text-slate-800">
                Hello, {greeting}
              </span>
              <IconChevron className="hidden h-4 w-4 shrink-0 text-slate-400 sm:block" />
            </div>
          </div>
        </header>

        <main className="mx-auto w-full max-w-6xl flex-1 p-5 md:p-8">{children}</main>
      </div>
    </div>
  );
}
